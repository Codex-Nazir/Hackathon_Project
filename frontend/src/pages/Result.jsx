      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="w-full md:w-2/3 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <DataCard title="Extracted (OCR)" data={result.extractedData} />
            <DataCard title="Database Record" data={result.originalRecord || {}} />
          </div>
        </div>
      </div>
    </div>
...
const DataCard = ({ title, data }) => (
  <div className="glass-card p-6 border border-white/5">
    <h5 className="font-bold text-gray-400 mb-4 border-b border-white/5 pb-2">{title}</h5>
    <div className="space-y-3">
      {Object.entries(data).map(([key, value]) => {
        if (key === '_id' || key === '__v' || key === 'qrCode' || key === 'pdfPath' || key === 'hash') return null;
        return (
